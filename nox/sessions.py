...
class SessionRunner:
    ...
    def get_direct_dependencies(
        ...
        Returns:
            Iterable[~nox.session.SessionRunner]
            ...
            if self.func.venv not in sessions_by_signature:
                ...
                if self.func.venv in parametrized_sessions_by_name:
                    raise ValueError(
                        ...
                        " session that has multiple venvs. You probably meant to use"
                        f" venv='{self.func.venv}-{{python}}'."
                    )
                else:
                    raise KeyError(f"Session not found: {self.func.venv}")
            else:
            ...
            for requirement in self.func.requires:
                if requirement in sessions_by_signature:
                    ...
                    try:
                        ...
                        raise KeyError(f"Session not found: {exc.args[0]}")
        else:
        ...
        for dependency in self.get_direct_dependencies():
            if not dependency.result:
                self.result = Result(
                    self,
                    Status.ABORTED,
                    reason=f"Prerequisite session {dependency} was not successful",
                )
                return self.result
