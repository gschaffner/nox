...
def session_decorator(
    ...
    if venv is not None:
        for param_name in ("reuse_venv", "venv_backend", "venv_params"):
            var = locals()[param_name]
            if var is not None:
                raise ValueError(
                    f"The {param_name} argument to nox.session cannot be used when"
                    " using the venv argument."
                )
